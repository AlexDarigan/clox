...
static int addUpvalue(Compiler* compiler, uint8_t index, bool isLocal) {
	...
	if(upvalueCount == UINT8_COUNT) {
		perror("Too many closure variables in function");
		return 0;
	}
...
static void declareVariable() {
	...
	for(int i = current->localCount - 1; i >= 0; i--) {
		...
		if(identifierEquals(name, &local->name)) {
			errorAtCurrent("Already a variable with this name in scope");
		}
	}
	addLocal(*name);
}
...
static uint8_t argumentList() {
	...
	if(!check(TOKEN_RIGHT_PAREN)) {
		do {
			expression();
			if(argCount == 255) {
				perror("Can't have more than 255 arguments.");
			}
			argCount++;
...
static void super_(bool canAssign) {
	...
	if(currentClass == NULL) {
		perror("Can't use 'super' outside of a class");
	} else if(!currentClass->hasSuperclass) {
		perror("Can't use 'super' in a class with no superclass");
	}
...
static void this_(bool canAssign) {
	if(currentClass == NULL) {
		perror("Can't use 'this' outside of a class");
		return;
	}
	variable(false);
}
...
static void classDeclaration() {
	...
	if(match(TOKEN_LESS)) {
		...
		if(identifierEquals(&className, &parser.previous)) {
			perror("A class can't inherit from itself");
		}
...
static void returnStatement() {
	if(current->type == TYPE_SCRIPT) {
		perror("Can't return from top level code");
	}
	if(match(TOKEN_SEMICOLON)) {
		emitReturn();
	} else {
		if(current->type == TYPE_INITIALIZER) {
			perror("Can't return a value from an initializer");
		}
		expression();
